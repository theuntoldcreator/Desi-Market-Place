...
import { Phone, Loader2, Upload, X, MapPin, Info, Image as ImageIcon, AlertCircle, CheckCircle2 } from 'lucide-react';
...
interface ImagePreview {
  id: string;
  url: string;
  status: 'existing' | 'moderating' | 'safe' | 'blocked';
  file?: File;
  message?: string;
}
...
export function EditListing({ isOpen, onClose, listing }: EditListingProps) {
  ...
  useEffect(() => {
    if (listing) {
      ...
      setImagePreviews((listing.image_urls || []).map((url: string) => ({ id: url, url, status: 'existing' } as ImagePreview)));
      setImagesToDelete([]);
    }
  }, [listing]);
  ...
  const updateListingMutation = useMutation({
    mutationFn: async () => {
      ...
      const newImagesToUpload = imagePreviews.filter(p => p.status === 'safe' && p.file).map(p => p.file as File);
      ...
      const existingImageUrls = imagePreviews.filter(p => p.status === 'existing').map(p => p.url);
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="w-screen h-dvh max-w-full p-0 gap-0 rounded-none sm:max-w-2xl sm:h-auto sm:max-h-[95vh] sm:rounded-lg flex flex-col overflow-hidden [&>button]:hidden">
        <div className="flex items-center justify-between p-4 border-b sticky top-0 bg-background/95 backdrop-blur-sm z-10 pt-[calc(env(safe-area-inset-top)+1rem)] sm:pt-4">
          <div className="space-y-1"><DialogTitle className="text-xl">Edit Listing</DialogTitle><DialogDescription>Update the details for your item.</DialogDescription></div>
        ...
        <div className="flex-grow overflow-y-auto hide-scrollbar">
          <div className="p-4 space-y-6">
              ...
              <div className="grid grid-cols-3 sm:grid-cols-5 gap-3">
                {imagePreviews.map((preview) => (
                  <div key={preview.id} className="relative group aspect-square">
                    <img src={preview.url} alt="Listing image" className="w-full h-full object-cover rounded-lg border" />
            ...
            <div className="space-y-4 pt-4 border-t"><Label className="font-semibold">Contact Info *</Label><div className="flex items-center gap-2"><Select value={formData.countryCode} onValueChange={(value) => setFormData(prev => ({ ...prev, countryCode: value }))}><SelectTrigger className="w-[100px]"><SelectValue /></SelectTrigger><SelectContent>{countries.map(c => <SelectItem key={c.code} value={c.dial_code}>{`${c.code} ${c.dial_code}`}</SelectItem>)}</SelectContent></Select><div className="relative flex-1"><Phone className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" /><Input id="contact" value={formData.phoneNumber} onChange={(e) => setFormData(prev => ({ ...prev, phoneNumber: e.target.value }))} placeholder="Phone Number" className="pl-10" /></div></div></div>
          </div>
        </div>
        <DialogFooter className="p-4 border-t bg-background sticky bottom-0 z-10 pb-[calc(env(safe-area-inset-bottom)+1rem)] sm:pb-4">
          ...
          <Button onClick={() => updateListingMutation.mutate()} disabled={updateListingMutation.isPending || isModerating} className="w-full sm:w-auto">
            {isModerating ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Analyzing...</> : updateListingMutation.isPending ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Saving...</> : 'Save Changes'}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
