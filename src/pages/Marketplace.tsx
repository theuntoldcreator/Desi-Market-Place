...
export default function Marketplace() {
  ...
  const renderContent = () => {
    if (isLoading) return <div className="grid grid-cols-2 md:grid-cols-1 gap-6">{Array.from({ length: 8 }).map((_, i) => <Card key={i} className="p-0"><div className="h-96 bg-muted animate-pulse rounded-lg"></div></Card>)}</div>;
